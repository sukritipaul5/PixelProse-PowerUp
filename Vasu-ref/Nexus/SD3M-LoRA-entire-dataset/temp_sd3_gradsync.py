...
def dump_gradients(model, rank, iteration):
    for name, param in model.named_parameters():
        if param.requires_grad and param.grad is not None:
            ...
            filename = f"grad_{name}_rank{rank}_iter{iteration}.npy"
            np.save(os.path.join(args.output_dir, filename), gradient)

def compare_gradients(output_dir, num_processes):
    for filename in os.listdir(output_dir):
        if filename.startswith("grad_") and filename.endswith(".npy"):
            gradients = [np.load(os.path.join(output_dir, f"grad_{filename.split('_')[1]}_rank{i}_iter0.npy")) for i in range(num_processes)]
            max_diff = np.max([np.max(np.abs(grad - gradients[0])) for grad in gradients[1:]])
            logger.info(f"Max gradient difference for {filename}: {max_diff}")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):          
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                #Gradient Sync checks
                if global_step == 0 and args.gradient_accumulation_steps == 1:  # Only check on the first step
                    # Dump gradients
                    dump_gradients(transformer, accelerator.process_index, global_step)
                    # Sync point
                    accelerator.wait_for_everyone()
                    # On rank 0, compare gradients
                    if accelerator.is_main_process:
                        compare_gradients(args.output_dir, accelerator.num_processes)
