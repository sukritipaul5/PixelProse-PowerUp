...
class WebDatasetwithCachedLatents(IterableDataset):
    ...
    def process_sample(self,sample):
        ...
        try:
            ...
            tokenized_vlm_captions = []
            #tokenized_vlm_captions = defaultdict(list)
            for idx,tokenizer in enumerate(self.tokenizers):
                ...
                tokenized_vlm_captions.append(tokenized_caption)
                # tokenized_vlm_captions['input_ids'].append(tokenized_caption.input_ids.squeeze())
                # tokenized_vlm_captions['attention_mask'].append(tokenized_caption.attention_mask.squeeze())
                # tokenized_vlm_captions['tokenizer_num'].append(idx)
...
def custom_collate_fn(batch):
    ...
    collated = {}
    """
        Prompt_embeds and pooled_prompt_embeds mostly not used here. 
    """
    ...
    #collated['tokenized_vlm_captions'] = [item[3] for item in batch]
    tokenized_captions = [item[3] for item in batch]
    tokenized_captions = reorganize_batch(tokenized_captions)
    collated['tokenized_vlm_captions'] = tokenized_captions
