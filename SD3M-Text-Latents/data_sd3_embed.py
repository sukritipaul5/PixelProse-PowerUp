...
class StatefulWebDataset(IterableDataset):
    ...
    def _create_datapipe(self):
        ...
        def group_data(data):
            ...
            json_data = next((d for d in data if d[0].endswith('.json')), None)
            if image_data and json_data:
                return (image_data[0],image_data[1], json_data[1])
            return None

        datapipe = Batcher(datapipe, 3)  # Group by 3 as each sample has .jpg, .txt, and .json
        ...
        def read_stream_wrapper(sample):
            if sample is None:
                return None
            file_name,image_stream, json_stream = sample
            image_data = image_stream.read()
            json_data = json_stream.read()
            return (file_name,image_data, json_data)
    ...
    def process_sample(self, sample):
        ...
        file_name,image_data, json_data = sample
        try:
