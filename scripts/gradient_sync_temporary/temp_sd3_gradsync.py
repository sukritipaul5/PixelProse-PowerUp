...
#Added for gradient syncing

def dump_gradients(model, rank, iteration):
    try:
        for name, param in model.named_parameters():
            if param.requires_grad and param.grad is not None:
                gradient = param.grad.data.cpu().numpy()
                # Change the filename format
                filename = f"grad_{name.replace('.', '_')}_rank{rank}_iter{iteration}.npy"
                np.save(os.path.join(args.output_dir, filename), gradient)
        logger.info(f"Gradients dumped for rank {rank} at iteration {iteration}")
    #No errors seen in the logs
    except Exception as e:
        logger.error(f"Error dumping gradients for rank {rank}: {str(e)}")

def compare_gradients(output_dir, num_processes, step):
    try:
        results = {}
        for filename in os.listdir(output_dir):
            if filename.startswith(f"grad_") and filename.endswith(f"_iter{step}.npy"):
                # Extract the parameter name from the filename
                param_name = filename.split('_rank')[0][5:]  # Remove 'grad_' prefix
                gradients = []
                for i in range(num_processes):
                    file_path = os.path.join(output_dir, f"grad_{param_name}_rank{i}_iter{step}.npy")
                    if os.path.exists(file_path):
                        gradients.append(np.load(file_path))
                    else:
                        logger.warning(f"Missing gradient file for rank {i}: {file_path}")
                
                if len(gradients) > 1:
                    max_diff = np.max([np.max(np.abs(grad - gradients[0])) for grad in gradients[1:]])
                    mean_diff = np.mean([np.mean(np.abs(grad - gradients[0])) for grad in gradients[1:]])
                    #equality = [np.all(grad==gradients[0]) for grad in gradients[1:]]
                    results[param_name] = {"max_diff": float(max_diff), "mean_diff": float(mean_diff)} 
                    #,\#'equality':equality}
        
        with open(os.path.join(output_dir, f"gradient_comparison_step{step}.json"), "w") as f:
            json.dump(results, f, indent=2)
        
        logger.info(f"Gradient comparison results saved for step {step}")
    except Exception as e:
        logger.error(f"Error comparing gradients: {str(e)}")
...
def parse_args(input_args=None):
    ...
    type=int,
    default=500,
    help="Frequency of gradient checks"
    )
...
def main(args):
    ...
    accelerator.register_save_state_pre_hook(utils_sd3.save_model_hook)
    accelerator.register_load_state_pre_hook(utils_sd3.load_model_hook)
    ...
    def compute_text_embeddings(prompts, text_encoders, tokenizers):
        with torch.no_grad():
            prompt_embeds, pooled_prompt_embeds = utils_sd3.encode_prompt(
    ...
    #Old
    # if getattr(train_dataset, 'custom_instance_prompts', True):
    #     # For MDS dataset, we don't pre-compute embeddings because each image has a unique prompt
    #     logger.info("Using custom prompts for each image.")
    #     prompt_embeds = None
    #     pooled_prompt_embeds = None
    ...
    # else:
    #     tokens_one = utils_sd3.tokenize_prompt(tokenizer_one, args.instance_prompt)
    #     tokens_two = utils_sd3.tokenize_prompt(tokenizer_two, args.instance_prompt)
    #     tokens_three = utils_sd3.tokenize_prompt(tokenizer_three, args.instance_prompt)
    ...
    if args.resume_from_checkpoint:
        if args.resume_from_checkpoint != "latest":
            ...
            dirs = [f for f in os.listdir(args.output_dir) if f.startswith("checkpoint-")]
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):   
            logs = {"loss": None, "lr": lr_scheduler.get_last_lr()[0]}       
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                if global_step % args.gradient_check_frequency == 0:
                    dump_gradients(transformer, accelerator.process_index, global_step)
                    accelerator.wait_for_everyone()
                    if accelerator.is_main_process:
                        compare_gradients(args.output_dir, accelerator.num_processes, global_step)
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.distributed_type == DistributedType.DEEPSPEED or accelerator.is_main_process:
                    if global_step % args.checkpointing_steps == 0:
                        ...
                        utils_sd3.save_lora_weights(save_path,global_step,transformer,accelerator)               
                #accelerator.log(logs, step=global_step)
