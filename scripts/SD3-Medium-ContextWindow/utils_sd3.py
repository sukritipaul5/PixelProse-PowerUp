...
def encode_prompt(
    ...
    for tokenizer, text_encoder in zip(clip_tokenizers, clip_text_encoders):
        prompt_embeds, pooled_prompt_embeds = _encode_prompt_with_clip(
            ...
            max_length=77,  # Keep this at 77 for CLIP
        )
...
def validate_log(args, accelerator, vae, text_encoders, transformer, global_step, weight_dtype, logger,max_prompt_length=None):
    ...
    # Optionally truncate prompts to max_prompt_length
    if max_prompt_length:
        tokenizer = pipeline.tokenizer  # Assuming the pipeline has a tokenizer attribute
        prompts = [tokenizer.decode(tokenizer.encode(prompt, truncation=True, max_length=max_prompt_length)) for prompt in prompts]
