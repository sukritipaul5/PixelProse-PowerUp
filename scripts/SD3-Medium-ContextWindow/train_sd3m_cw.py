...
#
# This script has been modified to support fine-tuning SD3-M without LoRA.
# by Tom's Lab.
#
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default="/lus/eagle/projects/DemocAI/sukriti/sd3m-cw/validation_prompts.txt",
...
class ContextWindowMLP(nn.Module):
...
def main(args):
    ...
    if args.scale_lr:
     ...
     #ADDED for context window mlp
    t5_embed_dim = 4096
    clip_length = 77
    max_length = 512

    #context_mlp = ContextWindowMLP(t5_embed_dim, t5_embed_dim)
    context_mlp = ContextWindowMLP(4096, 2048).to(accelerator.device, dtype=weight_dtype)
    #context_mlp.to(accelerator.device, dtype=weight_dtype)


    # Make sure the trainable params are in float32.
    if args.mixed_precision == "fp16":
        models = [transformer, context_mlp]
        if args.train_text_encoder:
    ...
    context_mlp_parameters = list(context_mlp.parameters())
    ...
    if args.train_text_encoder:
        ...
        # params_to_optimize = [
        #     transformer_parameters_with_lr,
        #     context_mlp_parameters
        # ]
        params_to_optimize = [
            {"params": transformer_lora_parameters, "lr": args.learning_rate},
            {"params": context_mlp_parameters, "lr": args.learning_rate}
        ]
    ...
    TAR_PATH = "/lus/eagle/projects/DemocAI/common-datasets/pixelprose-shards/*/*.tar"
    LATENTS_DIR = "/lus/eagle/projects/DemocAI/common-datasets/pixelprose-latents/"
    ...
    if args.train_text_encoder:
        (
            ...
            transformer, context_mlp, text_encoder_one, text_encoder_two, optimizer, train_dataloader, lr_scheduler
        )
        assert text_encoder_one is not None
        assert text_encoder_two is not None
        assert text_encoder_three is not None
    else:
        transformer, context_mlp, optimizer, train_dataloader, lr_scheduler = accelerator.prepare(
            transformer, context_mlp, optimizer, train_dataloader, lr_scheduler
        )
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                if (args.use_custom_prompts==True):
                    ...
                    for i, text_encoder in enumerate(text_encoders):
                        input_ids = tokenized_prompts["subset1"][i]["input_ids"].to(device=accelerator.device)
                        attention_mask = tokenized_prompts["subset1"][i]["attention_mask"].to(device=accelerator.device)
                        ...
                        if i < 2:  # CLIP text encoders
                            ...
                            subset1_embeds.append(outputs.hidden_states[-2])
                            pooled_embeds.append(outputs[0])
                        else:  # T5 text encoder
                            subset1_embeds.append(text_encoder(input_ids=input_ids, attention_mask=attention_mask)[0])

                    # Process subset2 with T5XXL only (for tokens 78 to 512)
                    subset2_input_ids = tokenized_prompts["subset2"][2]["input_ids"][:, :max_length-clip_length].to(device=accelerator.device)
                    subset2_attention_mask = tokenized_prompts["subset2"][2]["attention_mask"][:, :max_length-clip_length].to(device=accelerator.device)
                    subset2_embeds = text_encoders[2](
                        input_ids=subset2_input_ids,
                        attention_mask=subset2_attention_mask,
                    )[0]

                    # # Apply MLP to subset2 embeddings
                    # subset2_embeds_processed = context_mlp(subset2_embeds)

                    # Process the first 77 tokens as before
                    clip_embeds = torch.cat(subset1_embeds[:2], dim=-1)  # Shape: [1, 77, 2048]
                    t5_embeds = subset1_embeds[2]  # Shape: [1, 77, 4096]

                    # Process the extended context
                    extended_context = subset2_embeds  # Shape: [1, n, 4096] where n is the number of extended tokens
                    extended_context_processed = context_mlp(extended_context)  # Shape: [1, n, 2048]

                    # Debug prints
                    logger.info(f"clip_embeds shape: {clip_embeds.shape}")
                    logger.info(f"t5_embeds shape: {t5_embeds.shape}")
                    logger.info(f"extended_context shape: {extended_context.shape}")
                    logger.info(f"extended_context_processed shape: {extended_context_processed.shape}")

                    # Ensure correct shapes
                    clip_embeds = clip_embeds.view(1, 77, 2048)
                    t5_embeds = t5_embeds.view(1, 77, 4096)

                    # Create prompt_embeds tensor
                    prompt_embeds = torch.zeros(1, 154, 4096, device=clip_embeds.device, dtype=clip_embeds.dtype)

                    # Assign embeddings
                    prompt_embeds[:, :77, :2048] = clip_embeds
                    prompt_embeds[:, :77, 2048:] = t5_embeds[:, :, 2048:]  # Only take the last 2048 dimensions

                    # Process extended context
                    if extended_context_processed.shape[1] < 77:
                        padding = torch.zeros(1, 77 - extended_context_processed.shape[1], 2048, 
                                            device=extended_context_processed.device, 
                                            dtype=extended_context_processed.dtype)
                        extended_context_processed = torch.cat([extended_context_processed, padding], dim=1)

                    prompt_embeds[:, 77:, :2048] = extended_context_processed[:, :77, :]
                    pooled_prompt_embeds = torch.cat(pooled_embeds, dim=-1)  # Shape: [1, 2048]
                    pooled_prompt_embeds = pooled_prompt_embeds.squeeze(0)  # Shape: [2048]

                    # Final shapes
                    logger.info(f"Final prompt_embeds shape: {prompt_embeds.shape}")
                    logger.info(f"Final pooled_prompt_embeds shape: {pooled_prompt_embeds.shape}")
                ...
                if accelerator.sync_gradients:
                    parameters = [param for group in params_to_optimize for param in group["params"]]
                ...
                if accelerator.is_main_process:
                    ...
                    if condition:
                        ...
                        utils_sd3.validate_log(args,accelerator,vae,text_encoders,\
                            unwrapped_transformer,global_step,weight_dtype,logger=logger,max_prompt_length=args.max_sequence_length)
    ...
    if accelerator.is_main_process:
        ...
        utils_sd3.validate_log(args, accelerator, vae, text_encoders, unwrapped_transformer, global_step, weight_dtype, logger=logger,max_prompt_length=args.max_sequence_length)
