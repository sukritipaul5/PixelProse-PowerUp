...
import os
# os.environ['HF_TOKEN']='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'
...
class MDS_Dataset(StreamingDataset):
    def __init__(self, local, tokenizers, size=1024, center_crop=False, random_flip=False, max_length=77):
        super().__init__(
            local=local,
            remote=None,
            shuffle=True,
            ...
            shuffle_block_size=2_000_000
        )
        self.image_size = size
        self.center_crop = center_crop
        self.random_flip = random_flip
        self.tokenizers = tokenizers  # Changed from tokenizer to tokenizers
    ...
    def __getitem__(self, idx):
        ...
        if self.center_crop:
            ...
            y1, x1, h, w = self.train_crop.get_params(image, (self.image_size, self.image_size))
...
# class MDS_Dataset(StreamingDataset):
#     def __init__(self, local, size=1024, center_crop=False, random_flip=False):
#         super().__init__(
#             local=local,
#             remote=None,
#             shuffle=True,  # This handles shuffling within the dataset
#             batch_size=1,
#             num_canonical_nodes=2,  # Adjust this value as needed
#             shuffle_block_size=2_000_000  # Adjust this value as needed
#         )
#         self.image_size = size
#         self.center_crop = center_crop
#         self.random_flip = random_flip
...
def main(args):
    #wandb.init(project="sd3-medium-finetuning", name=args.output_dir)
    ...
    deepspeed_plugin = DeepSpeedPlugin(
        ...
        zero_stage=2,
        offload_optimizer_device="cpu",  # Adjust if needed
        offload_param_device="cpu",  # Adjust if needed
    )
    ...
    if args.optimizer.lower() == "prodigy":
        ...
        print("optimizer 1406")

    # Dataset and DataLoaders creation:

    train_dataset = MDS_Dataset(
        local=args.instance_data_dir,
        tokenizers=tokenizers,  
        size=args.resolution,
        #mid_epoch_resumption=True,
        center_crop=args.center_crop,
        random_flip=args.random_flip
    )
    print(f"Total number of samples in dataset: {len(train_dataset)}")
    ...
    if getattr(train_dataset, 'custom_instance_prompts', True):
        ...
        # This block is for non-custom prompts (not used with MDS dataset)
        if not args.train_text_encoder:
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):          
                        ...
                        if accelerator.is_main_process:
                            ...
                            if accelerator.is_main_process:
                                if global_step % args.checkpointing_steps == 0:     
                                    ...
                                    # print(f"Starting DeepSpeed save for step {global_step}")
                                    # save_path = os.path.join(args.output_dir, f"checkpoint-{global_step}")
                                    ...
                                    # # Get the DeepSpeed engine
                                    # ds_engine = accelerator.unwrap_model(transformer)
                                    
                                    # if isinstance(ds_engine, deepspeed.DeepSpeedEngine):
                                    #     # Save using DeepSpeed's checkpointing
                                    #     ds_engine.save_checkpoint(save_dir=save_path)
                                    #     print(f"Saved DeepSpeed checkpoint to {save_path}")
                                    # else:
                                    #     print("Not using DeepSpeed engine, falling back to manual save")
                                    #     # Your manual save code here
            ...
            #             logger.info(f"Starting checkpoint at step {global_step}")
            #             log_memory_usage()
            #             # _before_ saving state, check if this save would set us over the `checkpoints_total_limit`
            #             print(f"args.checkpoints_total_limit:{args.checkpoints_total_limit}")
            #             if args.checkpoints_total_limit is not None:
            #                 print("Ongoing...")
            #                 logger.info(f"Ongoing")
            ...
            #             print(f"Finished checkpoint at step {global_step}")
            #             logger.info(f"Finished checkpoint at step {global_step}")
            #             log_memory_usage()
            #             save_path = os.path.join(args.output_dir, f"checkpoint-{global_step}")
